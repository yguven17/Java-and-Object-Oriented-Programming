...
public class Lab4Main {
	...
	public static void TestA() {
		...
		if (!cedi.getFullname().equals("Cedi Osman") || akin.getGender().compareTo(Athlete.Gender.MALE) != 0
				|| akin.getSalary() == 0 || cedi.getJerseyNumber().compareTo("16") != 0 || cedi.getOnContract()) {
		...
		if (!Athlete.searchByName("Arda").get(0).getJerseyNumber().equals("7")
				|| Athlete.searchByName("Akin").size() != 2) {
		...
		if (!basketball.getCaptain().getJerseyNumber().equals(cedi.getJerseyNumber())
				|| football.getAthletes().size() != 5 || football.teamSalary() != 100000
				|| Athlete.searchByName("Berkin").get(0).getOnContract() || !cedi.getTeam().equals(basketball)
				|| !cedi.getOnContract()
		...
		System.out.print("- Testing Constructors and getter methods: ");
		if (!football.getClub().equals(besiktas) || besiktas.getYearEstablished() != 1903
				|| !besiktas.getName().equalsIgnoreCase("Besiktas") || fenerbahce.clubSalary() != 60000
	...
	/*
	 * TO BE IMPLEMENTED DURING THE LAB, UNCOMMENT AND IMPLEMENT public static void
	 * TestB() { }
	 */
	public static void TestB() {
		...
		athletes.add(new Athlete("6", "Joe", "Biden", Athlete.Gender.MALE, 25000));
		athletes.add(new Athlete("5", "Jacinda", "Ardern", Athlete.Gender.FEMALE, 13000));
		athletes.add(new Athlete("32", "Abraham", "Lincoln", Athlete.Gender.MALE, 7000));
		athletes.add(new Athlete("23", "Sibel", "Siber", Athlete.Gender.FEMALE, 30000));
		athletes.add(new Athlete("17", "Angela", "Merkel", Athlete.Gender.FEMALE, 40000));
		athletes.add(new Athlete("55", "Harry", "Truman", Athlete.Gender.MALE, 18000));
		athletes.add(new Athlete("9", "Richard", "Nixon", Athlete.Gender.MALE, 50000));
		...
		Club Canberra = new Club("Club Canberr", 1907);
		...
		System.out.printf("Number of female athletes: %d\n",Canberra.genderCount(Gender.FEMALE) );
		System.out.printf("Number of male athletes: %d\n", Canberra.genderCount(Gender.MALE));
		
		System.out.println();
		
		System.out.println(Ottowa);
		System.out.printf("Number of female athletes: %d\n",Ottowa.genderCount(Gender.FEMALE));
		System.out.printf("Number of male athletes: %d\n",Ottowa.genderCount(Gender.MALE));
