...
public class DeckOfCards {
	...
	public boolean isFlush(int start) {
		...
		for (int count = 0; count < deck.length - 1; count++) {
			...
			if (deck[count].getSuit().equals(deck[count + 1].getSuit())) {
				start++;
				if (start == 4) {
					return true;
				}
		...
		// place holder for not getting syntax error - remove it to write yoru code.
	}
